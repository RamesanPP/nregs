...
<!-- about -->
	...
	<div class="container">
		<div class="price">
			<h3><?php echo $hd ?> :: Details</h3>
			<br />
		</div>
            <?php
            $seld=mysqli_query($dbcon,"select * from dadmin_data");
            $i=0;
		...
		<div class="<?php echo $cls ?>">
			<div class="business">
                            <a href="view.php?did=<?php echo $rd[2] ?>"><h2>
                                <?php
			...
			<ul>
                            <?php
                            $countp=mysqli_query($dbcon,"select * from padmin_data where sid='$rd[1]' and did='$rd[2]'");
                            $rp=mysqli_fetch_row($countp);
                            ?>
				<li><span><?php echo mysqli_num_rows($countp) ?></span> Panchayat</li>
                                <?php
                                $countnr=mysqli_query($dbcon,"select * from nregs where sid='$rd[1]' and did='$rd[2]'");
                                $rnr=mysqli_fetch_row($countnr);
                                ?>
				<li><span><?php echo mysqli_num_rows($countnr) ?></span> NREGS</li>
                                <?php
                                $countmem=mysqli_query($dbcon,"select * from nregs_member where sid='$rd[1]' and did='$rd[2]'");
                                ?>
				<li><span><?php echo mysqli_num_rows($countmem) ?></span> Members</li>
                                <?php
                                $countjob=mysqli_query($dbcon,"select * from job_data where sid='$rd[1]' and did='$rd[2]'");
                                ?>
				<li class="gd"><span><?php echo mysqli_num_rows($countjob) ?></span> Works</li>
			</ul>
			<div class="buy-me">
